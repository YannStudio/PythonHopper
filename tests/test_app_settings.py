...
def test_suggested_extensions_do_not_overlap_defaults():
    default_patterns = {
        pat.lower()
        for ext in DEFAULT_FILE_EXTENSIONS
        for pat in ext.patterns
    }
    for _group_label, options in SUGGESTED_FILE_EXTENSION_GROUPS:
        for option in options:
            assert default_patterns.isdisjoint({p.lower() for p in option.patterns})
