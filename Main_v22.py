...
import os, sys, json, csv, shutil, argparse, tempfile, datetime, threading, re
...
# ReportLab (PDF). Script werkt ook zonder (PDF wordt dan overgeslagen).
try:
...
SUPPORTED_EXTS = [".pdf", ".dxf", ".dwg", ".step", ".stp"]
    ...
    for enc in encodings:
        try:
            ...
            try:
                ...
                return pd.read_csv(filepath, sep=dialect.delimiter, encoding=enc, on_bad_lines="skip")
            except Exception:
...
def load_bom(path: str) -> pd.DataFrame:
    ...
    opp_col = find_any(["Oppervlakte", "Oppervlakte (m²)", "Oppervlakte (m2)", "Oppervlakte m²", "Oppervlakte m2",
                        "Area", "Area (m2)", "Area (m²)"])
    if opp_col is None:
    ...
    df["Materiaal"]   = "" if mat_col is None else df[mat_col].astype(str).fillna("").str.strip()

    if "Bestanden gevonden" not in df.columns: df["Bestanden gevonden"] = ""
    if "Status" not in df.columns: df["Status"] = ""

    return df[[
        "PartNumber","Description","Production",
        "Bestanden gevonden","Status",
        "Materiaal","Aantal","Oppervlakte","Gewicht"
    ]].copy()
...
def generate_pdf_order_platypus(path: str,
    ...
    doc = SimpleDocTemplate(
        path, pagesize=A4,
        leftMargin=margin, rightMargin=margin, topMargin=20*mm, bottomMargin=20*mm
    )
    width, height = A4
    ...
    # Adres: Adres_1, Adres_2 als één regel
    addr_line = None
    ...
    if addr_line: supp_lines.append(addr_line)
    if not addr_line:
        pc_gem = " ".join(x for x in [supplier.postcode, supplier.gemeente] if x)
        if pc_gem: supp_lines.append(pc_gem)
        if supplier.land: supp_lines.append(supplier.land)

    supp_lines.append(f"BTW: {supplier.btw or ''}")
    if supplier.contact_sales: supp_lines.append(f"Contact sales: {supplier.contact_sales}")
    if supplier.sales_email:   supp_lines.append(f"Sales e-mail: {supplier.sales_email}")
    if supplier.phone:         supp_lines.append(f"Tel: {supplier.phone}")
    ...
    def wrap_cell_html(val: str, small=False, align=None):
        style = ParagraphStyle(
            ...
            wordWrap="CJK",  # nette wrap voor omschrijving
        )
        if align:
    ...
    for it in items:
        ...
        mat = _material_nowrap(it.get("Materiaal",""))  # no wrap
        qty = int(pd.to_numeric(_to_str(it.get("Aantal","")).strip() or 1, errors="coerce") or 1)
    ...
    # Iets smaller voor Aantal; extra kolom Materiaal toegevoegd.
    ...
    tbl.setStyle(TableStyle([
        ...
        ("FONTSIZE", (0,0), (-1,0), 10),
        ("VALIGN", (0,0), (-1,-1), "TOP"),
        ("ALIGN", (3,1), (5,-1), "RIGHT"),  # Aantal, m², kg rechts
...
def write_order_excel(path: str, items: List[Dict[str,str]]) -> None:
    df = pd.DataFrame(items, columns=["PartNumber","Description","Materiaal","Aantal","Oppervlakte","Gewicht"])
    df.to_excel(path, index=False, engine="openpyxl")


# --------------------------- Copy per production & orders ---------------------------

def pick_supplier_for_production(prod: str, db: SuppliersDB, override_map: Dict[str,str]) -> Supplier:
...
def copy_per_production_and_orders(source: str,
                                   dest: str,
                                   bom_df: pd.DataFrame,
                                   ...
                                   footer_note: str = "") -> Tuple[int, Dict[str, str]]:
    ...
    for prod, rows in prod_to_rows.items():
        ...
        for row in rows:
            items.append({
                ...
                "Aantal": int(pd.to_numeric(_to_str(row.get("Aantal","") or 1), errors="coerce") or 1),
        ...
        try:
            generate_pdf_order_platypus(pdf_path, company, supplier, prod, items, footer_note=footer_note or DEFAULT_FOOTER_NOTE)
        except Exception as e:
